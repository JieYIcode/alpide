...
namespace Focal {
  ...
  private:
    ...
    // Distribution of ctrl in half-module
    std::array<ControlInitiatorSocket, Focal::OUTER_CHIPS_PER_FOCAL_OUTER_MODULE> socket_control_out;
  };

  ///@brief A module for Focal consisting of OUTER_CHIPS_PER_INNER_MODULE chips in OB configuration
  struct FocalOuterLongModule : public sc_module
  {
    FocalOuterLongModule(sc_core::sc_module_name const &name,
                  Detector::DetectorPosition pos,
                  Detector::t_position_to_global_chip_id_func position_to_global_chip_id_func,
                  const AlpideConfig& cfg);
  ...
  private:
    ...
    // Distribution of ctrl in half-module
    std::array<ControlInitiatorSocket, Focal::OUTER_CHIPS_PER_FOCAL_INNER_MODULE> socket_control_out;
  };
  ...
  private:
    ...
    std::shared_ptr<Focal::FocalOuterLongModule> mOuterModule[OUTER_MODULES_PER_INNER_STAVE];
  };
